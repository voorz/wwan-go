	...
	go func() {
		...
		for update := range updates {
			...
			if err := info.UnmarshalBinary(update.Value.InformationBuffer); err != nil {
				sendError(fmt.Errorf("decoding USSD event: %w", err))
				return
			}
...
func encodeUSSD(text string) (uint32, []byte, error) {
	...
	if gsm7Err == nil {
		...
		if len(packed) > 160 {
			return 0, nil, fmt.Errorf("encoding USSD: payload length %d exceeds 160", len(packed))
		}
		return ussdDCSGSM7, packed, nil
	}
	payload, err := sms.UCS2(text).MarshalBinary()
	if err != nil {
		return 0, nil, fmt.Errorf("encoding USSD: %w", err)
	}
	if len(payload) > 160 {
		return 0, nil, fmt.Errorf("encoding USSD: payload length %d exceeds 160", len(payload))
	}
...
func ussdMessage(info mbimproto.USSDInfo) (USSDMessage, error) {
	...
	default:
		if len(info.Payload) != 0 {
			return USSDMessage{}, fmt.Errorf("decoding USSD: data coding scheme %#x is not supported", info.DataCodingScheme)
		}
	}
	return message, nil
}
